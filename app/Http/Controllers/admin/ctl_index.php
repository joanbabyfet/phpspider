...
class ctl_index extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        //获取用户信息
        $admin = $this->repo_admin_user->find(['where' => [['username', '=', $credentials['username']]]]);
        if(empty($admin)) //帐号不存在
        {
            //写入登录日志
            $this->repo_admin_user_login_log->save([
        ...
        if($admin->status === mod_admin_user::DISABLE) //检测帐号是否禁用
        {
        ...
        if (!$token = auth($this->guard)->attempt($credentials))
        {
            ...
            return res_error(trans('api.api_login_pass_incorrect'), -4);
        }
        ...
        $this->repo_admin_user->bind_token_uid($token, $user['id'], 14 * 86400);
        ...
        $this->repo_admin_user->set_cache($user, $user['id'], 14 * 86400);
    ...
    public function logout()
    {
        ...
        //干掉用户信息缓存
        $this->repo_admin_user->del_cache($uid);
        //干掉token缓存
        $token = $token ? $token : $this->repo_admin_user->get_token_by_uid($uid);
        $this->repo_admin_user->unbind_token_uid($token, $uid);
...
//    }
    ...
    public function edit_pwd(Request $request)
    {
        ...
        $status = $this->repo_admin_user->edit_pwd([
            'id'            => $id,
        ...
        //寫入日志
        $this->repo_admin_user_oplog->add_log("修改密码 {$id}", $this->module_id);
