...
class ctl_user extends Controller
{
    ...
    {
        $agent_id   = $request->input('agent_id', ''); //渠道代理
        ...
        $realname     = $request->input('realname'); //用户名
        $username     = $request->input('username'); //帐号
...
//    }
    ...
    public function enable(Request $request)
    {
        ...
        //寫入日志
        $this->repo_admin_user_oplog->add_log("用户解封 {$id}", $this->module_id);
    ...
    public function disable(Request $request)
    {
        ...
        //寫入日志
        $this->repo_admin_user_oplog->add_log("用户封禁 {$id}", $this->module_id);
    ...
    public function update_amount(Request $request)
    {
        ...
        $type   = $request->input('type', 0); //交易类型 1=充值 2=提现
        $amount = $request->input('amount', 0); //充值金额
        $remark = $request->input('remark', ''); //备注(必填)
        if(empty($id) || empty($type) || empty($amount) || empty($remark))
        {
            return res_error(trans('api.api_param_error'), -1);
        }

        //获取用户信息
        $user = $this->repo_user->find(['where' => [['id', '=', $id]]]);
        if(empty($user))
        {
            return res_error('用户不存在', -2);
        }
        ...
        //寫入日志
        $this->repo_admin_user_oplog->add_log("修改额度 ", $this->module_id);

        return res_success([], trans('api.api_submit_success'));
    }
}
